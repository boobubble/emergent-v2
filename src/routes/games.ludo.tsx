...
function GamesPage() {
  ...
  if (gameIdFromUrl) return <ActiveGameView gameId={gameIdFromUrl} onLeave={() => navigate({ to: "/games/ludo", search: { id: undefined } })} />;
      ...
      <div className="mx-auto max-w-3xl space-y-4 px-4 pt-5">
        ...
        {game.status === "finished" && (
          <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-6 text-center">
            ...
            <Link to="/games/ludo" search={{ id: undefined }} className="mt-4 inline-block rounded-full bg-primary px-4 py-2 text-sm font-bold text-primary-foreground">Back to lobby</Link>
          </div>
        )}
      </div>
