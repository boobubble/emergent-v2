...
import {
  ...
  Flame, Sparkles, Trophy, Heart, Sun, Moon, LogIn, UserPlus, Eye,
} from "lucide-react";
...
import {
  HERO_DEFAULTS, HERO_SETTINGS_KEY, type HeroConfig, type HeroShowcaseItem,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && data?.value) {
        setCfg({ ...HERO_DEFAULTS, ...(data.value as Partial<HeroConfig>) });
      }
    })();
...
function HeroHomepage() {
  ...
  return (
    <div className={`min-h-screen ${bg} relative overflow-x-hidden`}>
      ...
      {/* HERO */}
      <section className="relative z-10 mx-auto max-w-7xl px-5 pt-8 pb-16 sm:pt-16">
      ...
      <Showcase
        ...
        tag="Chatrooms 💬"
        title="Realtime chat that actually feels alive"
        image={cfg.chatroomImageUrl}
        items={cfg.chatroomFeatures}
      />
      ...
      <Showcase
        ...
        tag="Social Feed 📰"
        title="A feed built for community, not algorithms"
        image={cfg.feedImageUrl}
        items={cfg.feedFeatures}
        reverse
      />
      ...
      <Showcase
        ...
        tag="Live Radio 🎙️"
        title="Tune into your favorite community DJs"
        image={cfg.radioImageUrl}
        items={cfg.radioFeatures}
      />
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <Glass key={s.label} className="p-5">
              <div className="flex items-center gap-2 text-xs opacity-70">{s.icon} {s.label}</div>
              <div className="mt-3 text-4xl">{s.emoji}</div>
              <div className="mt-2 text-sm opacity-70">Discover the community's most loved {s.label.toLowerCase()}.</div>
            </Glass>
          ))}
        </div>
      </section>
