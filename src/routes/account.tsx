...
import { ArrowLeft, Upload, Trash2, Save, LogOut, Coins, Flame, Trophy, Award, UserX, UserMinus, MessageCircle } from "lucide-react";
...
export const Route = createFileRoute("/account")({
  head: () => ({
    meta: [
      { title: "Account Settings — Palrgo" },
      { name: "description", content: "Update your profile, avatar, username and manage friends." },
      { property: "og:title", content: "Account Settings — Palrgo" },
      { property: "og:description", content: "Manage your Palrgo profile, friends and blocked users." },
    ],
  }),
...
function AccountPage() {
  ...
  const save = () => {
    ...
    if (!/^[a-zA-Z0-9_-]{3,20}$/.test(trimmed)) {
      alert("Username must be 3–20 chars (letters, numbers, _ or -).");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-border bg-background/80 px-6 py-3 backdrop-blur-md">
      ...
      <main className="mx-auto max-w-3xl space-y-8 px-6 py-10">
        {/* Profile card */}
        <section className="rounded-3xl border border-border bg-card p-8" style={{ boxShadow: "var(--shadow-panel)" }}>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="flex items-center gap-1 rounded-full bg-primary/15 px-3 py-1 text-[11px] font-bold text-primary hover:bg-primary/25"
                >
                  <Upload className="h-3 w-3" /> Upload
                </button>
                {me.avatarUrl && (
                  <button
                    onClick={() => updateMe({ avatarUrl: undefined })}
                    className="flex items-center gap-1 rounded-full bg-destructive/15 px-3 py-1 text-[11px] font-bold text-destructive hover:bg-destructive/25"
                  >
            ...
            <div className="min-w-0 flex-1 space-y-4">
              ...
              <Field label="Bio">
                <textarea
                  ...
                  rows={3}
                  maxLength={160}
                  className="w-full resize-none rounded-xl border border-border bg-input px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
                  ...
                  {(["online", "away", "offline"] as const).map(s => (
                    <button
                      ...
                      className={`rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wider transition ${
                        status === s ? "bg-primary text-primary-foreground" : "bg-white/5 text-muted-foreground hover:bg-white/10"
                      }`}
                    >
                      {s}
                    </button>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  onClick={save}
                  className="flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-bold text-primary-foreground hover:opacity-90"
                >
        ...
        <section>
          ...
          {friends.length === 0 ? (
            <p className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">
              No friends yet. Click a user's name in chat to add them.
            </p>
          ) : (
            <ul className="grid gap-2">
              {friends.map(u => (
                <li key={u.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card px-3 py-2">
                  ...
                  <button
                    onClick={() => { startDM(u.id); navigate({ to: "/" }); }}
                  ...
                  <button
                    ...
                    className="rounded-full bg-white/5 p-1.5 text-muted-foreground hover:bg-destructive/20 hover:text-destructive"
                    title="Remove friend"
                  >
              ...
              {blocked.map(u => (
                <li key={u.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card px-3 py-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full bg-white/5 px-3 py-1 text-[11px] font-bold text-muted-foreground hover:text-foreground"
                  >
                    <UserX className="h-3 w-3" /> Unblock
                  </button>
                </li>
              ))}
            </ul>
