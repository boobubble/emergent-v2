...
// -------- DJ / RJ broadcast credentials (admin-only, e.g. AzuraCast) --------
export const getDjBroadcastCredentials = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    await assertAdmin(context.userId);
    const { data, error } = await supabaseAdmin
      .from("dj_broadcast_credentials")
      .select("*")
      .eq("id", true)
      .maybeSingle();
    if (error) throw new Error(error.message);
...
const djCredSchema = z.object({
  provider: z.string().min(1).max(32).default("azuracast"),
  host: z.string().max(255).nullable().optional(),
  port: z.number().int().min(1).max(65535).nullable().optional(),
  mount: z.string().max(120).nullable().optional(),
  station_shortcode: z.string().max(120).nullable().optional(),
  source_username: z.string().max(120).nullable().optional(),
  source_password: z.string().max(255).nullable().optional(),
  listen_url: z.string().max(500).nullable().optional(),
  dj_name: z.string().max(60).nullable().optional(),
  notes: z.string().max(1000).nullable().optional(),
});

export const saveDjBroadcastCredentials = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => djCredSchema.parse(input))
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId);
    const { error } = await supabaseAdmin
      .from("dj_broadcast_credentials")
