...
export function BlogModerateView({
  ...
  return (
    <div className="yz-blog space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Pending Blog Posts</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Review community submissions. Approving publishes the post with existing rules.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {posts.map((post) => (
          <article key={post.id} className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 max-h-64 overflow-y-auto rounded-xl border border-border bg-background px-4 py-3">
