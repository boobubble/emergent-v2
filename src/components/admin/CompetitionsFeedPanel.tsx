...
export function CompetitionsFeedPanel() {
  ...
  const saveM = useMutation({
    mutationFn: (patch: Partial<CompetitionsFeedSettings>) =>
      save({ data: { event_flags: patch.event_flags, target_chatrooms: patch.target_chatrooms } }),
    onSuccess: () => { toast.success("Saved"); qc.invalidateQueries({ queryKey: ["competitions-feed-settings"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const provisionM = useMutation({
    mutationFn: () => provision({}),
    onSuccess: (r) => { toast.success(r.existed ? "CompetitionsBot linked" : "CompetitionsBot created"); qc.invalidateQueries({ queryKey: ["competitions-feed-settings"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const announceM = useMutation({
    mutationFn: (v: { competitionId: string; kind: "competition_trending" | "competition_ending" }) => announce({ data: v }),
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <p className="text-xs text-muted-foreground">
            CompetitionsBot ID:{" "}
            <span className="font-mono">{state.competitions_bot_user_id ?? "not provisioned"}</span>
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Master FeedBot pipeline (rate-limit, dispatcher, cooldowns) is reused — this bot just owns competition-related posts.
          </p>
        </CardContent>
      </Card>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {COMPETITION_CATEGORY_KEYS.map((k) => (
              <label key={k} className="flex items-center justify-between gap-2 rounded border border-border/60 p-2 text-sm">
                ...
                <AdminToggle checked={!!state.event_flags[k]} onCheckedChange={(v: boolean) => toggleFlag(k, v)} />
              </label>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <p className="mb-2 text-xs text-muted-foreground">
            Shared with FeedBot targets — competition posts land in the same rooms you already selected on the FeedBot page.
          </p>
          <div className="grid gap-2 md:grid-cols-2">
            {(roomsQ.data ?? []).map((r) => (
              <label key={r.id} className="flex items-center justify-between gap-2 rounded border border-border/60 p-2 text-sm">
                <span>{r.name}</span>
                <AdminToggle checked={state.target_chatrooms.includes(r.id)} onCheckedChange={(v: boolean) => toggleRoom(r.id, v)} />
              </label>
            ))}
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <p className="mb-3 text-xs text-muted-foreground">
            Broadcast "Trending" or "Ending soon" for a specific competition. Rate-limited to once per hour per competition per category via dedupe keys.
          </p>
          <div className="space-y-2">
            {(compsQ.data ?? [])
              ...
              .slice(0, 25)
              .map((c) => (
                <div key={c.id} className="flex flex-wrap items-center gap-2 rounded border border-border/60 p-2">
                  <span className="flex-1 text-sm">{c.name}</span>
                  <Button size="sm" variant="outline" onClick={() => announceM.mutate({ competitionId: c.id, kind: "competition_trending" })}>
                    🔥 Trending
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => announceM.mutate({ competitionId: c.id, kind: "competition_ending" })}>
                    ⏳ Ending soon
                  </Button>
                </div>
              ))}
