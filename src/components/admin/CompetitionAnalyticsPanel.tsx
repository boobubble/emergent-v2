...
export function CompetitionAnalyticsPanel() {
        ...
        actions={
          <div className="flex items-center gap-2">
            {(["day", "week", "month", "all"] as Win[]).map((w) => (
              <Button key={w} size="sm" variant={win === w ? "default" : "outline"} onClick={() => setWin(w)}>
        ...
        <Card>
          ...
          <CardContent>
            {q.isLoading ? <Skeleton className="h-32 w-full" /> : (
              <BarList items={(d?.topCompetitions ?? []).map((c) => ({ label: c.name, value: c.votes, href: `/competitions/${c.slug ?? c.id}` }))} />
            )}
          </CardContent>
        </Card>
