  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
      {approved.map((p) => {
        ...
        return (
          <div key={p.id} className="flex flex-col items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-3 text-center backdrop-blur">
            ...
            {!hideCounts && (
              <Badge variant="secondary" className="text-xs">{p.vote_count} votes</Badge>
            )}
            <Button
              size="sm"
              className="w-full"
