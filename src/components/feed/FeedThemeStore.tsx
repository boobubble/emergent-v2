...
import { Coins, Check, Lock, Clock, Sparkles } from "lucide-react";
import {
  listFeedThemes,
  listMyUnlocks,
  unlockFeedTheme,
  activateFeedTheme,
  type FeedThemeRow,
  type UserFeedThemeRow,
  type FeedThemeKey,
} from "@/lib/feed-themes";
...
export function FeedThemeStore({ open, onOpenChange, activeTheme, onThemeChange }: Props) {
  const { user } = useAuth();
  const [themes, setThemes] = useState<FeedThemeRow[]>([]);
  const [unlocks, setUnlocks] = useState<UserFeedThemeRow[]>([]);
  ...
  const unlockMap = useMemo(() => {
    const m = new Map<string, UserFeedThemeRow>();
    for (const u of unlocks) m.set(u.theme_key, u);
    return m;
  }, [unlocks]);

  const isUnlocked = (t: FeedThemeRow) => {
  ...
  const handleUnlock = async (t: FeedThemeRow) => {
    if (!user?.id) return;
    setBusy(t.theme_key);
    try {
  ...
  const handleActivate = async (t: FeedThemeRow) => {
    setBusy(t.theme_key);
    try {
      await activateFeedTheme(t.theme_key);
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-2">
            <span>Unlock premium skins for your feed. Layout stays the same — only the look changes.</span>
          </DialogDescription>
                  ...
                  <div className="mt-3 flex gap-2">
                    {active ? (
                      ...
                      >
                        {coins < t.price_coins
                          ? "Not enough coins"
                          : `Unlock for ${t.price_coins.toLocaleString()}`}
                      </Button>
                    )}
                  </div>
                </div>
