      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
          <div className="flex items-center gap-3">
            {showBack ? (
              shell?.embedded ? (
                <button
                  type="button"
                  onClick={shell.closePoetryCompose}
                  className="rounded-md p-1.5 hover:bg-muted"
                  aria-label="Back to Poetry Hub"
                >
                  <ArrowLeft className="h-4 w-4" />
                </button>
              ) : (
                <Link to="/poetry" className="rounded-md p-1.5 hover:bg-muted"><ArrowLeft className="h-4 w-4" /></Link>
              )
            ) : (
              <Link to="/feed" className="rounded-md p-1.5 hover:bg-muted" aria-label="Home"><Home className="h-4 w-4" /></Link>
            )}
            <Link to="/poetry" className="flex items-center gap-2">
          ...
          <nav className="hidden items-center gap-1 text-sm md:flex">
            <Link to="/poetry" activeOptions={{ exact: true }} activeProps={{ className: "bg-primary/10 text-primary" }} className="rounded-md px-3 py-1.5 hover:bg-muted">Discover</Link>
            <Link to="/poetry/challenges" activeProps={{ className: "bg-primary/10 text-primary" }} className="inline-flex items-center gap-1 rounded-md px-3 py-1.5 hover:bg-muted">
