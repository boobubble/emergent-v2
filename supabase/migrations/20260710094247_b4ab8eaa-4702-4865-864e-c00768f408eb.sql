-- Fix SQL exporter: detect table dependencies via function body scan
-- (pg_depend does not record body-referenced tables for PL/pgSQL functions,
--  so functions like bump_page_view that UPDATE public.custom_pages were
--  incorrectly classified as pre-table and emitted before the table existed.)
...
BEGIN
  ...
  -- Pre-table functions: no dependency on any public table (from pg_depend
  -- OR from a scan of the function body). Topo-sorted by function calls.
