-- =====================================================================
-- Rebuild admin_export_schema_sql with pg_dump-style dependency ordering
-- =====================================================================
...
BEGIN
  ...
  out := '-- ============================================' || nl
      ...
      || '-- Order: extensions -> schemas -> enum/domain/composite -> sequences ->' || nl
      || '--        pre-table functions -> tables -> post-table functions ->' || nl
      || '--        constraints -> indexes -> triggers -> RLS -> policies ->' || nl
      || '--        views -> materialized views -> grants' || nl
      || '-- ============================================' || nl2
  ...
  -- Schema
  out := out || '-- ---------- Schemas ----------' || nl
             || 'CREATE SCHEMA IF NOT EXISTS public;' || nl2;
  ...
  LOOP
    SELECT string_agg(quote_ident(a.attname) || ' ' || pg_catalog.format_type(a.atttypid, a.atttypmod),
                      ', ' ORDER BY a.attnum)
      INTO attrs FROM pg_attribute a
  ...
  -- Functions that DO NOT depend on any public table (topo-sorted by
  -- function->function calls). Emitted BEFORE tables so that table column
  -- defaults and CHECK expressions calling these functions resolve.
  ...
  FOR r IN
    WITH RECURSIVE funcs AS (
      ...
      FROM pg_proc p
      JOIN pg_namespace x ON x.oid = p.pronamespace
      WHERE x.nspname = 'public' AND p.prokind IN ('f','p')
    ), edges AS (
      -- pg_depend NORMAL edges between functions (recorded for SQL-language fns)
      ...
      WHERE f.oid <> g.oid
      UNION
      -- Body scan for qualified references: public.name(   OR "public".name(
      SELECT DISTINCT f.oid, g.oid
      FROM funcs f JOIN funcs g ON f.oid <> g.oid
      WHERE f.def_lc ~ ('(^|[^a-z0-9_])(public|"public")[.]("?)' || lower(g.proname) || '\3[[:space:]]*[(]')
      UNION
      -- Body scan for bare references (name follows non-identifier char, precedes '(')
      SELECT DISTINCT f.oid, g.oid
      FROM funcs f JOIN funcs g ON f.oid <> g.oid
    ...
    FROM funcs f
    WHERE NOT f.depends_on_table
    ORDER BY rank ASC, f.proname
  LOOP
    out := out || r.def || ';' || nl2;
  END LOOP;
  ...
  -- Functions that DO depend on tables (topo-sorted).
  ...
  -- Views (topo-sorted by view->view deps)
  out := out || '-- ---------- Views ----------' || nl;
  FOR r IN
    WITH RECURSIVE v AS (
  ...
  -- Materialized views (topo-sorted)
...
-- =====================================================================
-- Statement-level validator: run pre-split statements one at a time in
-- a throwaway schema and report the exact failing statement so the
-- client can compute file + line.
-- =====================================================================
CREATE OR REPLACE FUNCTION public.admin_validate_export_sql_stmts(_stmts text[])
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public','pg_catalog'
...
DECLARE
  ...
  i int;
  s text;
  err_message text; err_detail text; err_hint text;
  err_context text; err_sqlstate text;
BEGIN
  IF NOT public.is_admin(auth.uid()) THEN RAISE EXCEPTION 'forbidden'; END IF;
  EXECUTE format('CREATE SCHEMA %I', test_schema);
  EXECUTE 'SET LOCAL search_path = ' || quote_ident(test_schema) || ', pg_catalog';

  FOR i IN 1 .. COALESCE(array_length(_stmts, 1), 0) LOOP
    s := _stmts[i];
    IF s IS NULL OR btrim(s) = '' THEN CONTINUE; END IF;
    -- Rewrite public.* references into the throwaway schema so we can validate
    -- structure without touching real objects.
    s := replace(s, 'public.',   quote_ident(test_schema) || '.');
    s := replace(s, ' public ',  ' ' || quote_ident(test_schema) || ' ');
    s := replace(s, '''public''', quote_literal(test_schema));
    s := replace(s, '''public.', quote_literal(test_schema || '.'));
    BEGIN
      EXECUTE s;
    EXCEPTION WHEN OTHERS THEN
      ...
      RETURN jsonb_build_object(
        'ok', false,
        'failed_index', i,
        'failed_statement', left(_stmts[i], 800),
  ...
  EXECUTE format('DROP SCHEMA IF EXISTS %I CASCADE', test_schema);
  RETURN jsonb_build_object('ok', true, 'statements', COALESCE(array_length(_stmts,1),0));
END $function$;

REVOKE ALL ON FUNCTION public.admin_validate_export_sql_stmts(text[]) FROM PUBLIC;
