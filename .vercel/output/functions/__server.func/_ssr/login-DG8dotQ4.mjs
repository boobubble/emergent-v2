...
import { U as useHomePageMode, as as AuthScreen } from "./router-CYWPFaDK.mjs";
...
function LoginRoute() {
  ...
  reactExports.useEffect(() => {
    ...
    (async () => {
      try {
        const {
          data
        } = await supabase.from("app_settings").select("value").eq("key", "community_login_enabled").maybeSingle();
        if (cancel) return;
  ...
  if (state === "disabled") return /* @__PURE__ */ jsxRuntimeExports.jsx(Navigate, { to: homeMode === "hero" ? "/heropage" : "/welcome", replace: true });
