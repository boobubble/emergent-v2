...
import { a as Sparkles, w as Inbox, aj as Send, U as Users, N as Search, A as ArrowLeft, s as UserPlus, av as Ban, z as Check, X, as as UserMinus, bC as ShieldOff, _ as Clock } from "../_libs/lucide-react.mjs";
...
function FindFriendsPage() {
  ...
  reactExports.useEffect(() => {
    if (!meId) return;
    load();
    const ch = supabase.channel(`find-friends-${meId}`).on("postgres_changes", {
      event: "*",
      schema: "public",
      table: "friendships"
    }, () => load()).subscribe();
  ...
  const {
    ...
    return {
      friendsOf: friendsOf2,
      mineByOther: mineByOther2,
      blockedByMe: blockedByMe2,
      blockedMe: blockedMe2
    };
  }, [rows, meId]);
  const myFriends = friendsOf.get(meId) ?? /* @__PURE__ */ new Set();
  const allProfiles = reactExports.useMemo(() => Object.values(profiles).filter((p) => p.id !== meId && !p.isGuest), [profiles, meId]);
  const incoming = reactExports.useMemo(() => rows.filter((r) => r.status === "pending" && r.receiver_id === meId), [rows, meId]);
  const outgoing = reactExports.useMemo(() => rows.filter((r) => r.status === "pending" && r.sender_id === meId), [rows, meId]);
  const friendsList = reactExports.useMemo(() => allProfiles.filter((p) => myFriends.has(p.id)).sort(byOnlineThenName), [allProfiles, myFriends]);
  const suggestions = reactExports.useMemo(() => {
    return allProfiles.filter((p) => !myFriends.has(p.id) && !mineByOther.has(p.id) && !blockedByMe.has(p.id) && !blockedMe.has(p.id)).map((p) => {
  ...
  async function unfriend(otherId) {
    const row = rows.find((r) => r.status === "accepted" && (r.sender_id === meId && r.receiver_id === otherId || r.receiver_id === meId && r.sender_id === otherId));
    if (!row) return;
    const {
      error
  ...
  if (!user) return null;
  if (user.isGuest) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(GuestBlock, {});
  }
  const TABS = [{
    id: "suggestions",
    label: "Suggestions",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen bg-background text-foreground pb-10", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "sticky top-0 z-20 border-b border-border bg-card", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mx-auto flex max-w-[1100px] items-center gap-3 px-4 py-2.5", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/feed", className: "grid h-9 w-9 place-items-center rounded-full hover:bg-accent", "aria-label": "Back to feed", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ArrowLeft, { className: "h-5 w-5" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-base font-semibold sm:text-lg", children: "Find Friends" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "ml-auto text-xs text-muted-foreground hidden sm:block", children: [
        friendsList.length,
        " friend",
        friendsList.length === 1 ? "" : "s"
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mx-auto max-w-[1100px] px-3 py-4 sm:px-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-1 overflow-x-auto rounded-full bg-card p-1 shadow-sm border border-border", children: TABS.map((t) => {
        const Icon = t.icon;
        const active = tab === t.id;
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => setTab(t.id), className: `relative inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-3 py-1.5 text-xs sm:text-sm font-medium transition-colors ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Icon, { className: "h-3.5 w-3.5" }),
          " ",
          t.label,
          t.badge ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `ml-0.5 grid h-4 min-w-4 place-items-center rounded-full px-1 text-[10px] font-bold ${active ? "bg-primary-foreground/20 text-primary-foreground" : "bg-primary text-primary-foreground"}`, children: t.badge > 99 ? "99+" : t.badge }) : null
        ] }, t.id);
      }) }),
      tab === "search" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-3 flex items-center gap-2 rounded-full bg-card px-4 py-2 shadow-sm border border-border", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { className: "h-4 w-4 text-muted-foreground" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { autoFocus: true, value: q, onChange: (e) => setQ(e.target.value), placeholder: "Search by username…", className: "flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3", children: loading ? /* @__PURE__ */ jsxRuntimeExports.jsx(SkeletonGrid, {}) : tab === "suggestions" ? suggestions.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(Empty, { icon: Sparkles, text: "No suggestions right now. Check back later." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(CardGrid, { children: suggestions.map(({
        p,
        mutual
      }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(PersonCard, { p, subtitle: mutual > 0 ? `${mutual} mutual friend${mutual === 1 ? "" : "s"}` : p.status === "online" ? "Online now" : "New here", children: [
      ...
      ] }, p.id)) }) : tab === "requests" ? incoming.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(Empty, { icon: Inbox, text: "No incoming requests." }) : /* @__PURE__ */ jsxRuntimeExports.jsx(CardGrid, { children: incoming.map((r) => {
        const p = profiles[r.sender_id];
        if (!p) return null;
        const m = mutualWith(p.id);
        return /* @__PURE__ */ jsxRuntimeExports.jsxs(PersonCard, { p, subtitle: m > 0 ? `${m} mutual` : "Wants to be friends", children: [
        ...
        friendsList.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsx(PersonCard, { p, subtitle: p.status === "online" ? "Online" : p.lastSeen ? `Active ${timeAgo(p.lastSeen)}` : "Offline", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => unfriend(p.id), className: "btn-ghost", children: [
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsx(PersonCard, { p, subtitle: mutualWith(p.id) > 0 ? `${mutualWith(p.id)} mutual` : p.status === "online" ? "Online" : "Offline", children: isBlockedByMe ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => unblock(p.id), className: "btn-ghost", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ShieldOff, { className: "h-3.5 w-3.5" }),
          " Unblock"
...
function CardGrid({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3", children });
}
function PersonCard({
  p,
  subtitle,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(Link, { to: "/feed", className: "relative shrink-0", title: p.name, children: [
      p.avatarUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: p.avatarUrl, alt: p.name, loading: "lazy", decoding: "async", className: "h-12 w-12 rounded-full object-cover" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid h-12 w-12 place-items-center rounded-full text-sm font-semibold text-white", style: {
...
function Empty({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-2xl border border-border bg-card p-10 text-center shadow-sm", children: [
...
function SkeletonGrid() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3", children: Array.from({
    length: 6
  }).map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm", children: [
...
function GuestBlock() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid min-h-screen place-items-center bg-background p-6 text-center text-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-sm rounded-3xl border border-border bg-card p-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Clock, { className: "mx-auto h-7 w-7 text-muted-foreground" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "mt-3 text-lg font-bold", children: "Sign in to find friends" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: "Create an account to send and accept friend requests." }),
